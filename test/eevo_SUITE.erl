...
single_population_and_run(_Config) ->
	...
	ok = correct_stop_population(Population_Id),
	ok.
...
test_with_time_limit(_Config) ->
	...
	Test_Agent_Id = eevo:create_agent(
	...
	Population_Id = eevo:create_population(?time_limit(?TEST_POP_TIME_LIMIT)),
...
test_with_agents_limit(_Config) ->
	...
	Test_Agent_Id = eevo:create_agent(
		...
		_AgentProperties = test_agent:properties_example(),
		_MutationF = fun test_agent:mutationF_example/1),
	Population_Id = eevo:create_population(?agents_limit(?TEST_POP_AGENTS_LIMIT)),
	{ok, Gov_PId} = eevo:start_population(Population_Id),
	ok = eevo:add_agent(Population_Id, Test_Agent_Id),
	timer:sleep(1000),
	PopRun_Result = receive {run_end, Population_Id, Result} -> Result after 10 -> error(my_timeout) end,
...
test_with_score_limit(_Config) ->
	...
	Test_Agent_Id = eevo:create_agent(
		...
		_AgentProperties = test_agent:properties_example(),
		_MutationF = fun test_agent:mutationF_example/1),
	Population_Id = eevo:create_population(?score_limit(?TEST_POP_SCORE_LIMIT)),
	{ok, Gov_PId} = eevo:start_population(Population_Id),
	ok = eevo:add_agent(Population_Id, Test_Agent_Id),
	timer:sleep(1000),
	PopRun_Result = receive {run_end, Population_Id, Result} -> Result after 10 -> error(my_timeout) end,
